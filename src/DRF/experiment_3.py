...
from data_utils import get_spherical_data
from models import initialize_model, DeepMaternRandomPhaseS2RFFNN
from spherical_uq_methods import SphericalBayesianOptimizer
...
if __name__ == "__main__":
    ...
    if model_name == 'DeepMaternRandomPhaseS2RFFNN':
        ...
        train_dataset, val_dataset, val_dataset2, test_dataset,d_phi, d_theta = get_spherical_data(file_path,test_data_path, device=device)
        ...
        best_hyperparams, best_loss = optimizer.optimize(n_iterations=config['bayesian_optimization']['n_iterations'])
        ...
        def process_prediction_set(tensor_tuple):
            ...
            tensor_tuple = [torch.tensor(t) if isinstance(t, np.ndarray) else t for t in tensor_tuple]
        ...
        ssha_plot = ax.imshow(final_test_predictions.reshape(_NUM_LONGS, _NUM_LATS).T, origin='lower', cmap='coolwarm', extent=[0,360,-90,90], vmin=-0.25, vmax=0.25)
        ax.coastlines()
