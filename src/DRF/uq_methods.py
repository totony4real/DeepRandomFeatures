...
class DeepEnsemble:
    ...
    def map_loss(self, outputs, targets, model_parameters, s_squared, obs_noise):
        """
        ...
        """
        if obs_noise > 0:
            mse_term = ((outputs - targets) ** 2).sum() / obs_noise**2
        else:
            mse_term = ((outputs - targets) ** 2).sum() / (2 * 1e-2**2)
        reg_term = (
            0.5 * sum(param.pow(2).sum() for param in model_parameters) / s_squared
        )
    ...
    def train_and_evaluate(
        ...
        all_predictions, test_predictions, model = zip(*results)
        return (
            torch.stack(all_predictions).squeeze().to(self.device),
            test_predictions,
            model,
        )
    ...
    def _train_single_model(
        ...
        return torch.cat(predictions).unsqueeze(0), final_preds, model
