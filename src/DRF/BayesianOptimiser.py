...
class BayesianOptimiser:
    ...
    """
    def __init__(self, config, model_init_func, optimize_observation_noise=False,penalty_weight=0):
        """
        ...
        if self.optimize_observation_noise:
            self.observation_noise_bounds = torch.tensor(self.config['bayesian_optimization']['observation_noise_bounds']).float()
    ...
    def optimize(self, spatial_X_train, temporal_X_train, y_train, spatial_X_test, temporal_X_test):
        ...
        if self.optimize_observation_noise:
            ...
            train_x = torch.rand(n_initial_samples, num_hyperparameters - 1) * (self.bounds[:, 1] - self.bounds[:, 0]) + self.bounds[:, 0]
        else:
            train_x = torch.rand(n_initial_samples, num_hyperparameters) * (self.bounds[:, 1] - self.bounds[:, 0]) + self.bounds[:, 0]
        ...
        if self.optimize_observation_noise:
            ...
            observation_noise_samples = torch.rand(n_initial_samples, 1) * (self.observation_noise_bounds[1] - self.observation_noise_bounds[0]) + self.observation_noise_bounds[0]
        ...
        for iteration in range(self.config['bayesian_optimization']['n_iterations']):
            ...
            if self.optimize_observation_noise:
                ...
                print("self.observation_noise_bounds shape:", self.observation_noise_bounds.shape)
            ...
            if self.optimize_observation_noise:
                print(f"Iteration {iteration + 1}: Spatial Lengthscale = {new_x[0, 0].item()}, Temporal Lengthscale = {new_x[0, 1].item()}, Amplitude = {new_x[0, 2].item()}, Observation Noise = {new_x[0, 3].item()}, Loss = {new_y.item()}")
            else:
    ...
    def objective_function(self, hyperparams, spatial_X_train, temporal_X_train, y_train, spatial_X_test, temporal_X_test):
        ...
        observation_noise = hyperparams[3].item() if self.optimize_observation_noise else 0.00  # default if not optimizing
        ...
        all_predictions, test_predictions, trained_models = self.uq_method.train_and_evaluate(
            spatial_lengthscale, temporal_lengthscale, amplitude,
            train_loader, val_loader, spatial_X_test, temporal_X_test,obs_noise=observation_noise
        )
        ...
        mu, var, y_val, spatial_val, temporal_val, model=model_instance, batch_size=self.config['training']['batch_size'], gradient_penalty_weight=self.penalty_weight
    )
