...
class RFFLayer(nn.Module, ABC):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, amplitude=1.0):
        super().__init__()
    ...
    def initialize_layers(self):
        ...
        hidden_layer.weight.data = self.sample_from_spectral_density(
            hidden_layer.weight.shape
        )
    ...
    def forward(self, x):
        ...
        scaling_factor = torch.sqrt(
            torch.tensor(2.0 * self.amplitude**2 / self.hidden_layer.out_features)
        )
...
class SquaredExponentialRFFLayer(RFFLayer):
    ...
    def __init__(
        self, input_dim, hidden_dim, output_dim, lengthscale=1.0, amplitude=1.0
    ):
    ...
    def sample_from_spectral_density(self, shape):
        ...
        """
        return torch.distributions.Normal(loc=0, scale=1 / self.lengthscale).sample(
            shape
        )
...
class MaternRFFLayer(RFFLayer):
    ...
    def sample_from_spectral_density(self, shape):
        ...
        return torch.distributions.StudentT(
            df=2 * self.nu, scale=1 / self.lengthscale
        ).sample(shape)
...
class RandomPhaseFeatureMap:
    ...
    def __init__(self, num_features, nu, lengthscale, num_levels=24):
        ...
        spectrum = kernel._spectrum(
            sphere.get_eigenvalues(num_levels),
            nu=torch.tensor([nu]),
        ...
        self.levels = torch.multinomial(
            self.spectrum.squeeze(), num_features, replacement=True
        )
    ...
    def __call__(self, X):
        ...
        fX = (
            torch.sqrt(c)
            * self.gegenbauer12(self.levels, UXT)
            / np.sqrt(self.N * 0.079)
        )  # Shape (N, B)
...
if __name__ == "__main__":
    ...
    other_points = torch.stack(
        (
            torch.ravel(other_points_xs),
            torch.ravel(other_points_ys),
            torch.ravel(other_points_zs),
        ),
        dim=1,
    )

    # Initialise random phase S2 Matern network
    model = MaternRandomPhaseS2RFFLayer(
        hidden_dim=1000, output_dim=1, lengthscale=0.3, nu=3 / 2
    )
    ...
    norm = colors.Normalize(
        vmin=np.min(initial_sample), vmax=np.max(initial_sample), clip=False
    )
